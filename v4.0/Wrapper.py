r""" To compile the .ui file created with Qt Designer into a  Python file do the following:
...
from Tabs._09_Gatesweep_Lockin import Gatesweep_Lockin
from Tabs._10_Lock_In_Sweep import Lock_In_Sweep
from Tabs._11_Voltage_Bias import Voltage_Bias
from Tabs._12_Current_Bias import Current_Bias

from Tabs._14_Temperature_Scan import Temperature_Scan
...
class MyForm(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.connect(self.ui.actionTwo_Gates_with_Lock_in, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(8))
        self.connect(self.ui.actionLockInSweep, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(9))
        self.connect(self.ui.actionCurrent_Bias, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(10))
        self.connect(self.ui.actionVoltage_Bias, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(11))
        self.connect(self.ui.actionTemperature_Control, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(12))
        self.connect(self.ui.actionTemperature_Scan, SIGNAL('triggered()'), lambda : self.ui.stackedWidget.setCurrentIndex(13))
        ...
        self.Lock_In_Sweep = Lock_In_Sweep(main = self, ui = self.ui)
        
        self.KeithleyGateSweep = KeithleyGateSweep(main = self, ui = self.ui)
        
        self.Gatesweep_Lockin_program = Gatesweep_Lockin(main = self, ui = self.ui)
        
        self.Voltage_Bias = Voltage_Bias(main = self, ui = self.ui)
        
        self.Current_Bias = Current_Bias(main = self, ui = self.ui)
        
        self.Temperature_scan = Temperature_Scan(main = self, ui = self.ui)

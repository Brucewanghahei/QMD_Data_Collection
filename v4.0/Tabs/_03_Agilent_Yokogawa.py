...
class Agilent_Yokogawa():
    def __init__(self, main, ui):
        ...
        self.ui.pushButtonStart_ay.setDisabled(True)
        self.ui.pushButtoncloseVisa1_ay.setDisabled(True)
        self.ui.pushButtoncloseVisa2_ay.setDisabled(True)
        self.ui.pushButtonStop_ay.setEnabled(False)
        self.ui.pushButtonStart_ay.setEnabled(False)
        ...
        self.timeStep = .1
        self.ui.lineEditTimeStep_ay.setText(str(self.timeStep))
        ...
        main.connect(self.ui.pushButtonselectVisa_ay, SIGNAL('clicked()'), self.select_visa)
        main.connect(self.ui.pushButtonupdateVisa_ay, SIGNAL('clicked()'), self.update_visa)
        main.connect(self.ui.pushButtoncloseVisa1_ay, SIGNAL('clicked()'), self.close_visaCurrent)
        main.connect(self.ui.pushButtoncloseVisa2_ay, SIGNAL('clicked()'), self.close_visaVoltage)
        ...
        main.connect(self.ui.pushButtonStart_ay, SIGNAL('clicked()'), self.pre_start)
        main.connect(self.ui.pushButtonPause_ay, SIGNAL('clicked()'), self.collectDataThread.pause)
        main.connect(self.ui.pushButtonStop_ay, SIGNAL('clicked()'), self.stop)
        main.connect(self.ui.pushButtonClear_ay, SIGNAL('clicked()'), self.clear_plots)
        main.connect(self.ui.pushButtonFit_ay, SIGNAL('clicked()'), self.Fit)
        ...
        main.connect(self.ui.checkBox_dsave_ay, SIGNAL("clicked()"), self.Pre_dsave)
        main.connect(self.ui.pushButton_dsave_browse_ay, SIGNAL('clicked()'), self.Dsave_browse)
    ...
    def import_ay(self):
        ...
        if divider_found == True:
            ...
            self.ui.lineEditOutput_ay.setText("File was imported correctly.")
    ...
    def update_visa(self):
        ...
        self.ui.comboBoxSelectVisa1_ay.clear()
        self.ui.comboBoxSelectVisa2_ay.clear()
        
        check_current1 = False
        check_current2 = False

        for each_visa in visas:
            self.ui.comboBoxSelectVisa1_ay.addItem(each_visa)
            
        for each_visa in visas:
            self.ui.comboBoxSelectVisa2_ay.addItem(each_visa)
    ...
    def select_visa(self):
        self.rm = visa.ResourceManager()
        visa1 = str(self.ui.comboBoxSelectVisa1_ay.currentText())
        visa2 = str(self.ui.comboBoxSelectVisa2_ay.currentText())
        valid = False
        ...
        try:
            ...
            self.ui.lineEditError_ay.setText("Error In Selected Visas")
        ...
        if valid == True:
            ...
            self.ui.lineEditError_ay.setText("There are no errors")
            self.ui.pushButtonStart_ay.setDisabled(False)
            self.ui.pushButtonStop_ay.setDisabled(False)
            self.ui.pushButtoncloseVisa1_ay.setDisabled(False)
            self.ui.pushButtoncloseVisa2_ay.setDisabled(False)
            self.ui.groupBox_dsave_ay.setEnabled(True)
            self.ui.checkBox_dsave_ay.setEnabled(True)
        else:
            self.ui.labelVisa1.setText("None")
            self.ui.labelVisa2.setText("None")
            self.ui.pushButtonStart_ay.setDisabled(True)
            self.ui.pushButtoncloseVisa1_ay.setDisabled(True)
            self.ui.pushButtoncloseVisa2_ay.setDisabled(True)
            self.ui.lineEditError_ay.setText("Invalid Visas")
    ...
    def close_visaCurrent(self):
        ...
        self.ui.lineEditOutput_ay.setText("Visa For Current Has Been Closed")
        
    def close_visaVoltage(self):
        self.visaVoltage.close()
        self.ui.lineEditOutput_ay.setText("Visa For Voltage Has Been Closed")
    ...
    def pre_start(self):
        ...
        if len(self.Array)/self.repeat >= 200 and self.ui.radioButtonCurrent_mA_ay.isChecked():
    ...
    def start(self):
        self.ui.pushButtonStart_ay.setEnabled(False)
        self.ui.pushButtonPause_ay.setEnabled(True)
        self.ui.pushButtonStop_ay.setEnabled(True)
        self.dsave_filename = ''
        self.dsave_username = ''
        self.dsave_filetype = ''
        self.dsave_divide = ''
        
        if float(self.ui.lineEditTimeStep_ay.text()) > 0 or self.first_plot == True or self.clear_check == True:
            self.visaCurrent.write('OUTP ON')
            self.timeStep = float(self.ui.lineEditTimeStep_ay.text())
            if self.ui.checkBox_dsave_ay.isChecked():
                ...
                if self.dsave_dir_ok:
                    ...
                    if self.dsave_filename_ok:    
                        ...
                        if self.dsave_username_ok:
                            self.Dsave_filetype()
                            self.collectDataThread.input(self.ui, self.visaCurrent, self.visaVoltage, self.Array, self.timeStep, self.curve_item_ay, self.curve_item_vt_ay, self.curve_item_ct_ay, self.dsave_directory, self.dsave_filename, self.dsave_username, self.dsave_thread, self.dsave_filetype, self.dsave_divide)
                            self.ui.lineEditOutput_ay.setText("Running")
                        else:
                            self.ui.lineEditOutput_ay.setText('Enter user name for dynamic saving.')
                    else:
                        self.ui.lineEditOutput_ay.setText('Enter file name for dynamic saving.')
                else:
                    self.ui.lineEditOutput_ay.setText('Choose valid directory for dynamic saving.')
            else:
                self.collectDataThread.input(self.ui, self.visaCurrent, self.visaVoltage, self.Array, self.timeStep, self.curve_item_ay, self.curve_item_vt_ay, self.curve_item_ct_ay, self.dsave_directory, self.dsave_filename, self.dsave_username, self.dsave_thread, self.dsave_filetype, self.dsave_divide)
                self.ui.lineEditOutput_ay.setText("Running")
            self.ui.tabWidget_plot_ay.setCurrentIndex(2)
        else:
            self.ui.lineEditError_ay.setText("Error")
    ...
    def Analyze(self, current, voltage, steps, time, date_value):
        self.ui.pushButtonStart_ay.setEnabled(False)
        self.ui.pushButtonPause_ay.setEnabled(False)
        self.ui.pushButtonStop_ay.setEnabled(False)
        self.ui.pushButtonClear_ay.setEnabled(True)

        self.ui.tabWidget_save_ay.setEnabled(True)
        
        self.ui.lineEditOutput_ay.setText('Data Collection Is Done')
    ...
    def stop(self):
        ...
        self.ui.pushButtonStart_ay.setEnabled(True)
        self.ui.pushButtonStop_ay.setEnabled(False)
        self.ui.pushButtonPause_ay.setEnabled(False)
        self.ui.lineEditOutput_ay.setText("Stopped")
        self.ui.pushButtonPause_ay.setText("Pause")
    ...
    def clear_plots(self):
        ...
        self.ui.lineEditOutput_ay.setText("Plots have been cleared")
    ...
    def Fit(self):
            try:
                ...
                if self.ui.radioButtonTimeScale_ay.isChecked():
                    # start point
                ...
                elif self.ui.radioButtonStepScale_ay.isChecked():
                    # start point
                ...
                self.ui.lineEditOutput_ay.setText('Please enter valid start/end value')
                
    def Pre_dsave(self):
        if self.ui.checkBox_dsave_ay.isChecked():
            self.ui.label_dsave_directory_ay.setEnabled(True)
            self.ui.lineEdit_dsave_directory_ay.setEnabled(True)
            self.ui.pushButton_dsave_browse_ay.setEnabled(True)
            self.ui.groupBox_dsave_filename_ay.setEnabled(True)
            self.ui.radioButton_dsave_timename_ay.setEnabled(True)
            self.ui.radioButton_dsave_custname_ay.setEnabled(True)
            self.ui.lineEdit_dsave_custname_ay.setEnabled(True)
            self.ui.groupBox_dsave_filetype_ay.setEnabled(True)
            self.ui.radioButton_csv_ay.setEnabled(True)
            self.ui.radioButton_txt_ay.setEnabled(True)
            self.ui.label_dsave_username_ay.setEnabled(True)
            self.ui.lineEdit_dsave_username_ay.setEnabled(True)
            self.ui.label_dsave_comment_ay.setEnabled(True)
            self.ui.textEdit_dsave_comment_ay.setEnabled(True)
            self.ui.lineEditOutput_ay.setText("Dynamic saving opened.")
        else:
            self.ui.label_dsave_directory_ay.setEnabled(False)
            self.ui.lineEdit_dsave_directory_ay.setEnabled(False)
            self.ui.pushButton_dsave_browse_ay.setEnabled(False)
            self.ui.groupBox_dsave_filename_ay.setEnabled(False)
            self.ui.radioButton_dsave_timename_ay.setEnabled(False)
            self.ui.radioButton_dsave_custname_ay.setEnabled(False)
            self.ui.lineEdit_dsave_custname_ay.setEnabled(False)
            self.ui.groupBox_dsave_filetype_ay.setEnabled(False)
            self.ui.radioButton_csv_ay.setEnabled(False)
            self.ui.radioButton_txt_ay.setEnabled(False)
            self.ui.label_dsave_username_ay.setEnabled(False)
            self.ui.lineEdit_dsave_username_ay.setEnabled(False)
            self.ui.label_dsave_comment_ay.setEnabled(False)
            self.ui.textEdit_dsave_comment_ay.setEnabled(False)
            self.ui.lineEditOutput_ay.setText("Dynamic saving closed.")
    ...
    def Dsave_browse(self):
        ...
        if fileDir != '':
            ...
            self.ui.lineEdit_dsave_directory_ay.setText(fileDir)
            self.ui.lineEditOutput_ay.setText("Dynamic saving directory selected.")
            print self.dsave_directory
        else:
            self.ui.lineEdit_dsave_directory_ay.setText('None')
            self.ui.lineEditOutput_ay.setText('Choose a directory for dynamic saving.')
    
    def Dsave_directory(self):
        self.dsave_dir_ok = True
        if self.ui.lineEdit_dsave_directory_ay.text() == '' or self.ui.lineEdit_dsave_directory_ay.text() == 'None':
            self.dsave_dir_ok = False
        
    def Dsave_filename(self):
        self.dsave_filename_ok = True
        self.dsave_filename = ''
        if self.ui.radioButton_dsave_timename_ay.isChecked():
        ...
        elif self.ui.radioButton_dsave_custname_ay.isChecked():
            self.dsave_filename = str(self.ui.lineEdit_dsave_custname_ay.text())
            if self.dsave_filename == '':
                self.dsave_filename_ok = False
    
    def Dsave_filetype(self):
        if self.ui.radioButton_csv_ay.isChecked():
            self.dsave_filetype = '.csv'
            self.dsave_divide = ','
        elif self.ui.radioButton_txt_ay.isChecked():
    ...
    def Dsave_username(self):
        ...
        self.dsave_username = ''
        self.dsave_username = str(self.ui.lineEdit_dsave_username_ay.text())
    ...
    def G_save(self):
        if self.ui.radioButton_Custom_Name_G_ay.isChecked() and self.ui.lineEdit_Custom_Name_G_ay.text() == '':
            ...
            visa1_name = str(self.ui.labelVisa1.text())
            ...
            temp.append(str(self.ui.lineEditTimeStep_ay.text()))
            comments.append(temp)
            # Line 10: comments
            temp = []
    ...
    def O_save(self):
        if self.ui.comboBox_Name_Folder_O_ay.currentText() == 'None':
            self.ui.label_condition_O_ay.setText('Please choose a user name.')
...
class CollectData(QThread):
    ...
    def input(self, ui, visaCurrent, visaVoltage, Array, timeStep, curve, curve_vt, curve_ct, dsave_directory, dsave_filename, dsave_username, dsave_thread, dsave_filetype, dsave_divide):
        self.ui = ui
        ...
        if self.ui.radioButtonCurrent_nA_ay.isChecked():
            self.magnitude= ['nA', 1E-09]
        elif self.ui.radioButtonCurrent_uA_ay.isChecked():
            ...
            if float(time.time() - t1) > self.timeStep:
                ...
                if self.pauseLoop == False:
                    ...
                    if self.n == 0:
                        ...
                        self.date_value.append(self.date_and_time)
    ...
    def pause(self):
        if self.pauseLoop == True:
            ...
            self.pauseLoop = True
            self.ui.pushButtonPause_ay.setText('Continue')
            self.ui.lineEditOutput_ay.setText("Paused")
    ...
    def Pre_dynamic_save(self, num, is_last):
        if self.ui.checkBox_dsave_ay.isChecked():
            ...
            if is_last:
                ...
                data.append(self.time[num])
                data.append(num)
                data.append(self.Array[num])
                data.append(self.voltage[num])
                data.append(self.current[num])
                ...
                if num == 0:
                    ...
                    temp.append(str(self.ui.comboBoxSelectVisa1_ay.currentText()))
                    comments.append(temp)
                    #####################
                    temp = []
                    temp.append('Visa Name:')
                    visa = str(self.ui.labelVisa1.text())
                    ...
                    temp = []
                    temp.append('Comments:')
                    temp.append(str(self.ui.textEdit_dsave_comment_ay.toPlainText()))
                    comments.append(temp)
                    #####################
                    parameters.append('Date')
                    units.append('String')

...
class Array_Builder():
    ...
    def __init__(self, main, ui):
        ...
        main.connect(self.ui.pushButton_plot, SIGNAL('clicked()'), self.Plot_general)
        main.connect(self.ui.pushButton_clear, SIGNAL('clicked()'), self.Clear)
        main.connect(self.ui.pushButton_save, SIGNAL('clicked()'), self.Save)
        main.connect(self.ui.pushButton_browse_save, SIGNAL('clicked()'), self.Browse_save)
    ...
    def Plot_general(self):
        ...
        if self.ui.lineEdit_start.text() == '':
            self.ui.label_condition.setText('Please enter Starting Value.')
        elif self.ui.textEdit_peaks_steps.toPlainText() == '':
            self.ui.label_condition.setText('Please enter Peaks and Steps Value.')
        else:
            self.parameters = []
            self.Values = []
            ...
            for i in range(0, len(para)):
                if para[i] != '':
                    try:
                        ...
                        if len(para[i]) != 3:
                            self.ui.label_condition.setText('Invalid values entered. Please check line #' + str(i + 1) + '.')
                            first_available = False
                            break
                        elif para[i][1] != math.floor(para[i][1]) or para[i][1] < 1:
                            self.ui.label_condition.setText('Please enter an integer repeat value in line #' + str(i + 1) + '.')
                            first_available = False
                            break
                        else:
            ...
            if first_available:   
                ...
                if peak != start and step == 0:
                    self.ui.label_condition.setText('Invalid values entered. Please check line #' + str(1) + '.')
                else:
                    if peak < start and step > 0:
                        step = -1 * step
                    ...
                    self.Plot()
                    self.ui.label_condition.setText('Array has been plotted')
    ...
    def Plot(self):
        ...
        self.ui.label_step_number.setText(str(item))
    ...
    def Save(self):
        ...
        if file_name == '':
            self.ui.label_condition.setText("Please enter the valid array name.")
        else:
            if self.ui.lineEdit_directory_save.text() != "None":
                ...
                self.ui.label_condition.setText("File has been saved.")
            else:
                self.ui.label_condition.setText("Please enter the valid saving directory.")
    ...
    def Browse_save(self):
        ...
        if fileDir != '':
            ...
            self.ui.label_condition.setText("Please choose valid saving directory.") 
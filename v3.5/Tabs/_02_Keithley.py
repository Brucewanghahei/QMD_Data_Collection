...
class Keithley():
    ...
    def Copy_keithley(self):
        ...
        if Values != None:
            ...
            self.ui.groupBox_visa_keithley.setEnabled(True)
            self.ui.groupBox_scan_mode_keithley.setEnabled(True)
        else:
                ...
                if self.dsave_dir_ok:
                    ...
                    if self.dsave_filename_ok:    
                        ...
                        if self.dsave_username_ok:
                            ...
                            self.ui.lineEdit_condition_keithley.setText('Enter user name for dynamic saving.')
                    else:
                        self.ui.lineEdit_condition_keithley.setText('Enter file name for dynamic saving.')
                else:
                    self.ui.lineEdit_condition_keithley.setText('Choose valid directory for dynamic saving.')
            else:
                self.collect_data_thread.input(self.visa_chosen, self.ui, self.Array, self.go_on, self.curve_item, self.curve_ct_item, self.curve_vt_item, c_limit, self.dsave_directory, self.dsave_filename, self.dsave_username, self.dsave_thread, self.dsave_filetype, self.dsave_divide)
                self.ui.pushButton_pause_keithley.setEnabled(True)
                self.ui.pushButton_stop_keithley.setEnabled(True)
    ...
    def Pre_dsave(self):
        if self.ui.checkBoxK1_dsave.isChecked():
            ...
            self.ui.lineEdit_condition_keithley.setText("Dynamic saving opened.")
        else:
            ...
            self.ui.lineEdit_condition_keithley.setText("Dynamic saving closed.")
    ...
    def Dsave_browse(self):
        ...
        if fileDir != '':
            ...
            self.ui.lineEdit_condition_keithley.setText("Dynamic saving directory selected.")
        else:
            self.ui.lineEditK1_dsave_directory.setText('None')
            self.ui.lineEdit_condition_keithley.setText('Choose a directory for dynamic saving.')
            ...
            while True:
                if float(time.time() - t1_) > self.time_step:
                    ...
                    if self.go_on:
                        ...
                        #end_time = time.time()
                        self.voltage = self.inst.query('READ? "defbuffer2", SOUR')
                        
                        #end_time = time.time()
                        x_value.append(float(self.voltage))
                        self.x_plot = np.array(x_value)
                        self.x_plot = self.x_plot / self.voltage_scale[0]
                        self.current = self.inst.query('READ? "defbuffer1", READ')
                        y_value.append(float(self.current))
    ...
    def Pre_dynamic_save(self, num, is_last):
        if self.ui.checkBoxK1_dsave.isChecked():
            ...
            if is_last:
                ...
                data.append(self.voltage.strip('\n'))
                data.append(self.current.rstrip('\n'))
                ...
                if num == 0:
                    ...
                    temp = []
                    temp.append('Visa Name:')
                    visa = str(self.ui.lineEdit_visa_keithley.text())

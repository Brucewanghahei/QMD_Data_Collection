...
class Lock_In():
    ...
    def plotData(self, dataPoints, reading, magnitude):
        self.ui.labelLastReadingLI.setText(str(round(reading, 10)) + " V ")
        self.ui.lineEditSTDEVLI.setText(str(round(np.std(dataPoints), 10)) + " " + magnitude[0])
        self.ui.lineEditMaxMinLI.setText(str(round(np.max(dataPoints) - np.min(dataPoints), 10)) + " " + magnitude[0])
    ...
    def append_parameters(self):
        ...
        if self.collectDataThread.pauseLoop == False:
    ...
    def tableAppend(self):
        ...
        tableValues =[self.sens, self.frequency, self.amplitude, self.QFactor, self.TimeConstant, self.lastReading, self.STDEV, self.MaxMin]
                ...
                if collect_check == True:
                    ...
                    if valid == True:
                        ...
                        if self.ui.radioButtonTimeScaleLI.isChecked() == True:
                            self.ui.curvewidgetPlot.plot.set_titles("Lock-in Data", "Time (s)", "Voltage (" + magnitude[0] + ")")
                            scale = 'Time'
                        elif self.ui.radioButtonStepScaleLI.isChecked() == True:
                            self.ui.curvewidgetPlot.plot.set_titles("Lock- in Data", "Steps", "Voltage (" + magnitude[0] + ")")
                            scale = 'Steps'
                        ...
                        if check == True:
                            ...
                            self.Steps = np.append(self.Steps, len(self.dataY))
                            self.global_time = np.append(self.global_time, self.time_now)
                            self.dataX = self.global_time

                            if self.i_plot >= int(self.ui.lineEditPlotEveryLI.text()) and scale == 'Time':
                            ...
                            elif self.i_plot >=  int(self.ui.lineEditPlotEveryLI.text()) and scale == 'Steps':
                            ...
                            self.stop() 
                self.emit(SIGNAL("plot"), dataPoints, reading, magnitude)

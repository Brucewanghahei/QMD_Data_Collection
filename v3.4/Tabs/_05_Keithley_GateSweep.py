...
class Collect_data(QThread):
    ...
    def input(self, instruments, ui, Array, go_on, curves, curveWidgets):
    ...
    def stop(self):
        ...
        self.ui.lineEditk2_condition_keithley.setText('Stopped.')
    ...
    def pause(self):
        if self.pause_collecting:
            ...
            self.ui.lineEditk2_condition_keithley.setText('Paused. Click continue to run.')
    ...
    def run(self):
        ...
        for i in range(0, len(self.Array)):
            ...
            if not self.stop_collecting:
                ...
                gate_data = self.read_gate_voltage()
                self.gateVolt_data.append(float(gate_data[0]))
                self.gateCurr_data.append(float(gate_data[1]))
                resis_data = self.resistance_measurement_single(self.lead_voltage)
                self.curr_data.append(float(resis_data[1]))
                self.resistance_data.append(resis_data[0]/resis_data[1])
        ...
        self.ui.lineEditk2_condition_keithley.setText('Scan completed.')
        self.emit(SIGNAL("data_available"), date_value, t_value, self.stepData, self.Array, self.gateVolt_data, self.curr_data, self.gateCurr_data, self.resistance_data)
        self.MPL_Plot()

...
class Lock_In():
    ...
    def start(self):
        ...
        if self.radioButtonDynamicSaveLI.isChecked() == True:
        ...
        if self.clear_plot == True:
            self.ui.curvewidgetPlotLI.replot()
            self.clear_plot = False
...
class SaveData(QThread):
    ...
    def run(self):
        if self.final_save == True:
            ...
            f.write('Steps' +self.file_info[2] + 'Voltage(' + self.units[i] + ')' + '\n')
    ...
    def autoSave(self):
        ...
        if not os.path.isdir(self.file_info[3]):
            os.makedirs(self.file_info[3])
        f_name = self.file_info[3] + '\\' + 'Autosave session' + '' + str(self.i) + self.file_info[1]
        ...
        f.write(self.file_info[4] + '\n')               
        f.write('Steps' +self.file_info[2] + 'Voltage(' + self.units[self.i-1] + ')' + '\n')
